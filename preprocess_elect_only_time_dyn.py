...
def prep_data(data, nos_of_days,covariates,counties, stat_feats, data_start, train=True):
    ...
    windows_per_series = np.full((num_series), (time_len - input_size) // stride_size) # create 370(num_series)*1 dimensional vector
    #print("windows_per_series pre: ", windows_per_series.shape)
    if train: windows_per_series -= (data_start + stride_size - 1) // stride_size # hop through the entire time series with windows rain/test
    ...
    # for adj matrix
    #covariates = covariates.reshape(int(nos_of_days), counties * counties)
...
# civariate for simple time series
def gen_covariates(num_covariates):
    
    times =  pd.date_range(start='2020-08-02', end='2020-12-14')
...
#      i = -1 # keep track of nos of days
    
      
#     for day in csv_files: # nos of days of the covariate matrix ## change
...
#         for k in range(1, stat_feats):
#             covariates[:,:, k] = stats.zscore(covariates[:, :, k])
#     return covariates[:, :, :stat_feats]


#     ## with only adjacency matrix
#     i = -1
#     for day in csv_files: # nos of days
#         gcn_data = pd.read_csv(day)
#         i = i + 1
#         #data = gcn_data.iloc[0, 0]
#         for j in range(counties):
#             for k in range(counties):
#                 covariates[i,j,k] = gcn_data.iloc[j, k]
#             #covariates[i, j, 5] = input_time.month
    
#         for k in range(1, counties):
#             covariates[:,:, k] = stats.zscore(covariates[:, :, k])

#     return covariates[:, :, :counties]
...
if __name__ == '__main__':
    ...
    path = "/content/gdrive/My Drive/covariate_mat_300_xavier_CASES_MOBILITY_feats_threshold_new/"
    ...
    nos_of_days = 135 # simple time series
    #nos_of_days = 88 #number of days of the predictor ## NY covid death counts.
    ...
    colums = [col for col in range(counties)] ## Take 100 counties #first column being date
    data_frame = pd.read_csv(csv_path,usecols=colums)
    #print ("Data frame:", data_frame)
   
    
    
    # for dynamic 2 feats nodes
    data_frame = data_frame.iloc[0:135] # take 68 days from Oct 16
    #data_frame = data_frame.iloc[2:137]  # take 68 days from Oct 16
    #data_frame = data_frame.iloc[2:151]  # take nos of days of NYT predictor data(death counts)  from Oct 16
    print ("Shape of data_frame:", np.shape(data_frame))
    
    #data_frame = data_frame.iloc[:,[range(49)]]
    #data_frame = data_frame.drop(["Days"], axis = 1)
   ...
   # For simple time series
    ...
    #train_data = data_frame[2:130].values # take training data ~ condiitoning range
    ...
    # for simple time series
    # train_data = data_frame[2:92].values # take training data ~ condiitoning range
    # test_data = data_frame[82:102].values # test data ~ prediction range
