...
class Net(nn.Module):
    def __init__(self, params):
        ...
        # print ("Mew of Laplace >>>>>>>>>:", self.distribution_mu)
        #
        # self.distribution_presigma = nn.Linear(params.lstm_hidden_dim * params.lstm_layers, 1)
        #
        # print ("sigma of Distri >>>>>>>>>>>>>:", self.distribution_presigma)
        ...
        self.pi = nn.Sequential(
            nn.Linear(params.lstm_hidden_dim * params.lstm_layers, num_gaussians),
            nn.Softmax(dim=1)
        )
        #self.sigma = nn.Linear(in_features, out_features * num_gaussians)
        #self.mu = nn.Linear(in_features, out_features * num_gaussians)

        self.distribution_mu = nn.Linear(params.lstm_hidden_dim * params.lstm_layers, 1 * num_gaussians) ## 40 * 3




        self.distribution_presigma = nn.Linear(params.lstm_hidden_dim * params.lstm_layers, 1 * num_gaussians)

        print("sigma:", self.distribution_presigma)
        self.distribution_sigma = nn.Softplus()
    ...
    def forward(self, x, idx, hidden, cell):
        ...
        #x = x.repeat(1, 5, 1)
        #onehot_embed = onehot_embed.repeat(1,5,1)
        # #print("hidden in forward before:", hidden.shape)
        # hidden = hidden.repeat(1,5,1)
        # cell = cell.repeat(1, 5, 1)
        #cell = cell.repeat(1, 5, 1)
        #hidden = hidden.unsqueeze(1).expand_as(x)
        #cell = cell.unsqueeze(1).expand_as(x)
        # print("HIDDEN SHAPE:", hidden.shape)
        # print("CELL SHAPE:", cell.shape)
        # print("X SHAPE:", x.shape)
        # print ("ONE HOT SHAPE:", onehot_embed.shape)
        ...
        pi = self.pi(hidden_permute)
        pre_sigma = self.distribution_presigma(hidden_permute)
        sigma = torch.exp(self.distribution_sigma(pre_sigma))
        sigma = sigma.view(-1, self.num_gaussians, self.out_features)
        mu = self.distribution_mu(hidden_permute)
        mu = mu.view(-1, self.num_gaussians, self.out_features)
        #return torch.squeeze(pi),torch.squeeze(mu), torch.squeeze(sigma), hidden, cell
        return pi, sigma, mu,hidden,cell
    ...
    def gaussian_probability(sigma, mu, target):
        ...
        #print ("sigma:", sigma)
        #print("TARGET INITIAL:", target.shape)
        #print ("SIGMA :", sigma.shape)
        #target = target.repeat(5,1) #sigma = 5
        target = target.unsqueeze(1).expand_as(sigma)
        #print ("target shape after reshaping:", target.shape)
        #print("target after reshaping:", target.shape)

        # print ("mu shape in GP :", mu.shape)
        # print("target shape in GP :", target.shape)
        # print("sigma shape in GP :", sigma.shape)
    ...
    def test(self, x, v_batch, id_batch, hidden, cell, sampling=False):
        #x = x.repeat(1, 5, 1)  # for GMM
        #id_batch = id_batch.repeat(1,5)
        batch_size = x.shape[1]
        #batch_size = x.shape[1] * 5 ## for GMM
        # print ("X SHAPE IN TEST.:", x.shape)
        # print("BATCH SIZE IN TEST :", batch_size)
        # print("ID BATCH SIZE IN TEST :", id_batch.shape)


        if sampling:
            samples = torch.zeros(self.params.sample_times, batch_size, 5,self.params.predict_steps,
            ...
            for j in range(self.params.sample_times):
                ...
                for t in range(self.params.predict_steps):
                    ...
                    pi_de,mu_de, sigma_de, decoder_hidden, decoder_cell = self(x[self.params.predict_start + t].unsqueeze(0),id_batch, decoder_hidden, decoder_cell)

                    mu_de = torch.reshape(mu_de, (mu_de.size()[0], mu_de.size()[1]))
                    sigma_de = torch.reshape(sigma_de, (sigma_de.size()[0], sigma_de.size()[1]))

                    sample_mu = torch.median(samples, dim=0)[0]
                    sample_sigma = samples.std(dim=0)

                    sigma_de = sigma_de * pi_de
                    sigma_de = torch.mean(sigma_de, dim=1)

                    mu_de = mu_de * pi_de
                    mu_de = torch.mean(mu_de, dim=1)

                    mix = torch.distributions.Categorical(torch.ones(5,))
                    comp = torch.distributions.Normal(mu_de, sigma_de)
                    gmm = torch.distributions.MixtureSameFamily(mix, comp)


                    pred = gmm.sample()  # not scaled
                    print ("PRED:", pred)
            ...
            #print("sample_mu INITIAL(NOT SAMPLING):", sample_mu.shape)

            sample_sigma = torch.zeros(batch_size, self.params.predict_steps, device=self.params.device)

            sample_pi = torch.zeros(batch_size, self.params.predict_steps, device=self.params.device)
            ...
            for t in range(self.params.predict_steps):
                pi_de,mu_de, sigma_de, decoder_hidden, decoder_cell = self(x[self.params.predict_start + t].unsqueeze(0),
                                                                     id_batch, decoder_hidden, decoder_cell)

                # print ("INITIAL")
                #print("PI_DE SHAPE:", pi_de.shape)
                # print("mu_de shape :", mu_de.shape)
                # #print ("SIGMA DE INITIAL:", sigma_de)
                # print("sigma_de shape:", sigma_de.shape)
                #
                # print ("INITIAL")

                #tensor1.size()[1])
                mu_de = torch.reshape(mu_de, (mu_de.size()[0],mu_de.size()[1] ))
                #print("MU_DE SHAPE RESHAPE:", mu_de.shape)

                #sigma_de = torch.reshape(sigma_de, (200,))
                sigma_de = torch.reshape(sigma_de, (sigma_de.size()[0],sigma_de.size()[1] ))
                #print("SIGMA_DE RESHAPE:", mu_de.shape)


                #sample_mu = torch.reshape(sample_mu, (40, 4))
                #print ("SAMPLE MU RESHAPE:", sample_mu.shape)
                mu_de = mu_de * pi_de
                mu_de = torch.mean(mu_de,dim = 1)
                #print("MU DE SCALED:", (np.shape(mu_de)))
                #print ("MU DE SCALED:", (mu_de))
                sample_mu[:, t] = mu_de * v_batch[:, 0] + v_batch[:, 1]
                #sample_mu[:,:,:, t] = mu_de * v_batch[:,:, 0] + v_batch[:,:, 1]

                sigma_de = sigma_de * pi_de
                sigma_de = torch.mean(sigma_de, dim=1)
                #print("SIGMA DE SCALED:", (np.shape(sigma_de)))
                #print("SIGMA DE SCALED:", (sigma_de))
                #sample_sigma = torch.reshape(sample_sigma,(40,4))
                #print("SAMPLE SIGMA RESHAPE:", sample_sigma.shape)
                #sigma_de = torch.reshape(sigma_de, (40,))
                sample_sigma[:, t] = sigma_de * v_batch[:, 0]

                pi_de = torch.mean(pi_de, dim=1)
                #print("PI SHAPE IN EVALUATE:", pi.shape)
                sample_pi[:, t] = v_batch[:, 0] * pi_de

                #x = x.repeat(1, 5, 1)  # for GMM
                #id_batch = id_batch.repeat(1, 5)
                if t < (self.params.predict_steps - 1):
                    x[self.params.predict_start + t + 1, :, 0] = mu_de
            return sample_pi, sample_mu, sample_sigma
...
def loss_fn(pi: Variable,mu: Variable, sigma: Variable, labels: Variable):
    """Calculates the error, given the MoG parameters and the target
    The loss is the negative log likelihood of the data given the MoG
    parameters.
    """
    # print ("Pi in loss func:", pi.shape)
    # print("sigma in loss func:", sigma.shape)
    # print("mu in loss func:", mu.shape)
    ...
    prob = pi * Net.gaussian_probability(sigma, mu, labels)
    #print ("PROB:", prob)
    # Reinitialize model if likelihood prob reaches close to zero
    # if torch.isnan(prob).any():
    #     Net.__init__()
    nll = -torch.log(torch.sum(prob, dim=1))
    nll = nll[nll!=float('inf')]
    #print ("NLL:", nll)
